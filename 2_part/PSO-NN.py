...
class PSOOptimizer:
    ...
    def fitness_function(self, X, X_train, y_train):
        ...
        for particle in X:
            ...
            loss = self.nn.forward_prop(particle, X_batch, y_batch)
...
def second_main():
    ...
    pars_C1 = [0.1, 1, 1.5]
    pars_C2 = [0.1, 1, 1.5]
    pars_W = [0.1, 0.3, 0.7, 0.9]
    pars_SwarmSize = [30, 100, 300, 1000]
    batchsize = 60 # The number of data instances used by the fitness function

    for par_C1, par_C2, par_W, par_SwarmSize in itertools.product(pars_C1, pars_C2, pars_W, pars_SwarmSize):
